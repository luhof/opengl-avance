...

{
...

    {
        ...
        glGenTextures(5, m_textureObjects);
        std::vector<GLubyte> emptyData(m_nWindowWidth * m_nWindowHeight * 4, 0);
        for(int i=0; i<5; i++){
            glBindTexture(GL_TEXTURE_2D, m_textureObjects[i]);
            glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, m_nWindowWidth, m_nWindowHeight, m_GBufferTextureFormat[i], GL_UNSIGNED_BYTE, &emptyData[0]);
        }



        const auto objPath = m_AssetsRootPath / m_AppName / "crytek-sponza" / "sponza.obj";
        glmlv::ObjData data;
